...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
eval "$(~/.claude/skills/gstuck/output/gstack/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "salience"
  ~/.claude/skills/gstuck/output/gstack/bin/gstack-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
...
> **STOP.** Before running the startup-mode diagnostic (Phase 2A: operating principles, pushback patterns, and the six forcing questions), Read `~/.claude/skills/gstuck/output/gstack/office-hours/sections/phase-2a-startup-diagnostic.md` and execute it
...
> **STOP.** Before running the builder-mode brainstorm (Phase 2B: operating principles, the wild exemplar, and the generative questions), Read `~/.claude/skills/gstuck/output/gstack/office-hours/sections/phase-2b-builder-brainstorm.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `~/.claude/skills/gstuck/output/gstack/office-hours/sections/design-and-handoff.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```

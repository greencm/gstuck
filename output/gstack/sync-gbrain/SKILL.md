...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
`~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-sync.ts`.
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-detect 2>/dev/null
```
...
```bash
bun run ~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-sync.ts <user-args>
```

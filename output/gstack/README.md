...
```bash
(cd ~/.claude/skills/gstack && ./setup --team) && ~/.claude/skills/gstuck/output/gstack/bin/gstack-team-init required && git add .claude/ CLAUDE.md && git commit -m "require gstack for AI-assisted work"
```
...
```bash
git clone --single-branch --depth 1 https://github.com/greencm/gstuck.git ~/gstack
cd ~/gstack && ./setup
```
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-uninstall
```
...
rm -rf "${CODEX_HOME:-$HOME/.codex}/skills/gstuck/output/gstack"* 2>/dev/null
...
command path points into `.claude/skills/gstuck/output/gstack/`: the SessionStart auto-update
...
**Codex says "Skipped loading skill(s) due to invalid SKILL.md"?** Your Codex skill descriptions are stale. Fix: `cd "${CODEX_HOME:-$HOME/.codex}/skills/gstuck/output/gstack" && git pull && ./setup --host codex` — or for repo-local installs: `cd "$(readlink -f .agents/skills/gstack)" && git pull && ./setup --host codex`

...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-question-log '{"skill":"document-release","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
~/.claude/skills/gstuck/output/gstack/bin/gstack-redact --from-file /tmp/gstack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
```bash
NEW_TITLE=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```

...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**If A (Retry)**: re-run `~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-detect`
...
```bash
. ~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-install || exit $?

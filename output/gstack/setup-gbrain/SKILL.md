...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-detect
```
...
> **STOP.** Before running the Step 1.5 broken-engine remediation — Step 1's detect returned `gbrain_local_status` of `broken-db` or `broken-config` and no shortcut flag was passed, Read `~/.claude/skills/gstuck/output/gstack/setup-gbrain/sections/engine-remediation.md` and execute it
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-install
```
...
> **STOP.** Before initializing the brain in Step 4 — run ONLY the procedure for the path picked in Step 2 (Paths 1/2a/2b/3/4 or Switch; also holds the PAT scope disclosure that `--cleanup-orphans` re-uses), Read `~/.claude/skills/gstuck/output/gstack/setup-gbrain/sections/brain-init.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
current_tier=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.claude/skills/gstuck/output/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/gstuck/output/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-source-wireup --strict \
...
> **STOP.** Before running the Step 7.5 transcript & memory ingest gate on Paths 1, 2a, 2b, or 3 (Path 4 skips this section entirely — see the skeleton's skip note), Read `~/.claude/skills/gstuck/output/gstack/setup-gbrain/sections/transcript-gate.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before persisting the Step 8 `## GBrain Configuration` block to CLAUDE.md (and the Search Guidance block after Step 9 passes), Read `~/.claude/skills/gstuck/output/gstack/setup-gbrain/sections/claude-md-persist.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
_HASH=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.claude/skills/gstuck/output/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.claude/skills/gstuck/output/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/gstuck/output/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"

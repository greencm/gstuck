...
describe('team-init resolves GSTACK_ROOT across every host (#2500)', () => {
  ...
  test('enforcement no longer hardcodes the Claude path as the only gate', () => {
    expect(teamInit).not.toContain('test -d ~/.claude/skills/gstuck/output/gstack/bin');
    expect(teamInit).not.toContain('if [ ! -d "$HOME/.claude/skills/gstuck/output/gstack/bin" ]');
  });

...
describe('question-tuning registry path is absolute (#2489)', () => {
  test('claude host renders the installed registry path', () => {
    ...
    expect(out).toContain('`~/.claude/skills/gstuck/output/gstack/scripts/question-registry.ts`');
  });
  ...
  test('rendered SKILL.md carries the absolute path', () => {
    ...
    expect(rendered).toContain('~/.claude/skills/gstuck/output/gstack/scripts/question-registry.ts');

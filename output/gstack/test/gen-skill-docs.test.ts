...
describe('DESIGN_REVIEW_LITE extended with Codex', () => {
  ...
  test('design-checklist path uses installed gstack/review root (#2694)', () => {
    ...
    // After install the file lives at ~/.claude/skills/gstuck/output/gstack/review/design-checklist.md.
    ...
    expect(content).toContain('~/.claude/skills/gstuck/output/gstack/review/design-checklist.md');
...
describe('Codex generation (--host codex)', () => {
  ...
  test('sidecar paths resolve through $GSTACK_ROOT (not gstack-review/)', () => {
    ...
    // (~/.claude/skills/gstuck/output/gstack/review/...), which the codex path rewrite turns
  ...
  test('Claude output uses installed-root review paths (#2518)', () => {
    ...
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('~/.claude/skills/gstuck/output/gstack/review/checklist.md');

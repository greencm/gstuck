...
describe('Codex generation (--host codex)', () => {
  ...
  test('Codex preamble resolves runtime assets from repo-local or global gstack roots', () => {
    ...
    expect(content).not.toContain('~/.codex/skills/gstuck/output/gstack/bin/gstack-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/gstuck/output/gstack/review/ (not gstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/gstack-review
    // but setup puts sidecars under .agents/skills/gstuck/output/gstack/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gstack-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use gstack/review/ path
    expect(content).toContain('.agents/skills/gstuck/output/gstack/review/checklist.md');
    expect(content).toContain('.agents/skills/gstuck/output/gstack/review/design-checklist.md');
  ...
  test('sidecar paths in ship skill point to gstack/review/ for pre-landing review', () => {
    ...
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/gstuck/output/gstack/review/');
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/gstuck/output/gstack/review/greptile-triage.md');
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 3: .claude/skills/review → .agents/skills/gstuck/output/gstack/review

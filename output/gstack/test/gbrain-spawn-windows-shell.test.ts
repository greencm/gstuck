...
describe("bashScriptInvocation", () => {
  ...
  test("POSIX execs the script directly, no interpreter needed", () => {
    const inv = bashScriptInvocation("/home/u/.claude/skills/gstuck/output/gstack/bin/gstack-brain-sync", ["--once"], {
      platform: "linux",
    });
    expect(inv).toEqual({
      cmd: "/home/u/.claude/skills/gstuck/output/gstack/bin/gstack-brain-sync",
      argv: ["--once"],
      shell: false,
    });
  });

...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    ...
    expect(HOST_PATHS.claude.binDir).toBe('~/.claude/skills/gstuck/output/gstack/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.claude/skills/gstuck/output/gstack/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.claude/skills/gstuck/output/gstack/design/dist');
  });

...
describe('gstack-artifacts-url', () => {
  test('--to ssh from canonical https', () => {
    const r = run(['--to', 'ssh', 'https://github.com/greencm/gstuck-artifacts-garrytan']);
    expect(r.code).toBe(0);
    expect(r.stdout).toBe('git@github.com:garrytan/gstack-artifacts-garrytan.git');
  });

  test('--to ssh from https-with-.git', () => {
    const r = run(['--to', 'ssh', 'https://github.com/greencm/gstuck-artifacts-garrytan.git']);
    expect(r.stdout).toBe('git@github.com:garrytan/gstack-artifacts-garrytan.git');
  });

  test('--to https is idempotent on https input', () => {
    const r = run(['--to', 'https', 'https://github.com/greencm/gstuck-artifacts-garrytan']);
    expect(r.stdout).toBe('https://github.com/greencm/gstuck-artifacts-garrytan');
  });
  ...
  test('--owner-repo extracts the path segment', () => {
    expect(run(['--owner-repo', 'https://github.com/greencm/gstuck-artifacts-garrytan']).stdout)
  ...
  test('round-trip: https → ssh → https is identity', () => {
    const original = 'https://github.com/greencm/gstuck-artifacts-garrytan';

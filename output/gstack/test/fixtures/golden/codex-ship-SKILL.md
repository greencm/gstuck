...
if [ -d ".agents/skills/gstack" ] && [ ! -L ".agents/skills/gstack" ]; then
  if [ -f ".agents/skills/gstuck/output/gstack/VERSION" ] || [ -d ".agents/skills/gstuck/output/gstack/.git" ]; then
    _VENDORED="yes"
  fi
...
> This project has gstack vendored in `.agents/skills/gstuck/output/gstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .agents/skills/gstuck/output/gstack/`
2. Run `echo '.agents/skills/gstuck/output/gstack/' >> .gitignore`
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/gstuck/output/gstack/review/TODOS-format.md`).
...
1. Read `.agents/skills/gstuck/output/gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.agents/skills/gstuck/output/gstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.agents/skills/gstuck/output/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
Read `.agents/skills/gstuck/output/gstack/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.agents/skills/gstuck/output/gstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>

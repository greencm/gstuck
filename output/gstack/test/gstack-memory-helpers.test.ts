...
describe("canonicalizeRemote", () => {
  it("strips https scheme and .git suffix", () => {
    expect(canonicalizeRemote("https://github.com/greencm/gstuck.git")).toBe("github.com/greencm/gstuck");
  });

  it("normalizes git@host:path scp-style remotes", () => {
    expect(canonicalizeRemote("git@github.com:garrytan/gstack.git")).toBe("github.com/greencm/gstuck");
  });
  ...
  it("produces the same key with or without a trailing slash", () => {
    expect(canonicalizeRemote("https://github.com/greencm/gstuck.git/")).toBe(
      canonicalizeRemote("https://github.com/greencm/gstuck.git")
    );
  });

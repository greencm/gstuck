## gstuck (based on upstream v1.57.10.0, sanitized 2026-06-18)
...
The section-path rewrite is surgical: only `~/.claude/skills/gstuck/output/gstack/<skill>/sections/`
...
#### For contributors
- `#1882` (skills hardcode `~/.claude/skills/gstuck/output/gstack/`, breaking non-`gstack` install
...
|---|---|---|
| `./setup` from worktree A with global install present | Leaks `~/.claude/skills/gstuck/output/gstack/A → workspaces/gstack/A` | Skipped with remediation hint |
...
| Source id derived for `github.com/greencm/gstuck` | `gstack-code-github.com-garrytan-gstack` (38 chars, contains `.`, fails gbrain `[a-z0-9-]{1,32}` validator) | `gstack-code-garrytan-gstack` (27 chars, valid) | 100% of github-hosted repos go from rejected to accepted |
...
- `bin/gstack-paths`: bash helper that resolves `GSTACK_STATE_ROOT`, `PLAN_ROOT`, `TMP_ROOT` with explicit fallback chains. Sourced via `eval "$(~/.claude/skills/gstuck/output/gstack/bin/gstack-paths)"`. Honors `GSTACK_HOME` → `CLAUDE_PLUGIN_DATA` → `$HOME/.gstack` → `.gstack`; `GSTACK_PLAN_DIR` → `CLAUDE_PLANS_DIR` → `$HOME/.claude/plans` → `.claude/plans`; `TMPDIR` → `TMP` → `.gstack/tmp`. Best-effort `mkdir -p` on tmp root; never fails the eval. Pattern matches existing `bin/gstack-slug` and `bin/gstack-codex-probe`.
...
- **Routing tests now work in CI.** Skills are installed at top-level `.claude/skills/` instead of nested under `.claude/skills/gstuck/output/gstack/`. project-level skill discovery doesn't recurse into subdirectories.
...
- Legacy fallback to `~/.claude/skills/gstuck/output/gstack/browse/src/server.ts`

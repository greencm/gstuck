...
[ -z "$_AUTO" ] && _AUTO=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/gstuck/output/gstack/bin/gstack-config set update_check true` to re-enable."
...
git clone --depth 1 https://github.com/greencm/gstuck.git "$TMP_DIR/gstack" || { echo "ERROR: clone failed — aborting upgrade (install untouched)." >&2; rm -rf "$TMP_DIR"; exit 1; }
...
_TEAM_MODE=$(~/.claude/skills/gstuck/output/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .claude/skills/gstuck/output/gstack/ 2>/dev/null || true
if ! grep -qF '.claude/skills/gstuck/output/gstack/' .gitignore 2>/dev/null; then
  echo '.claude/skills/gstuck/output/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.claude/skills/gstuck/output/gstack/` when you're ready."
...
```bash
~/.claude/skills/gstuck/output/gstack/bin/gstack-update-check --force 2>/dev/null || \
.claude/skills/gstuck/output/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.claude/skills/gstuck/output/gstack/` when you're ready."

...
# ~/.claude/skills/gstuck/output/gstack/. A user's own /checkpoint skill (regular file,
...
# --- Shape 2: ~/.claude/skills/gstuck/output/gstack/checkpoint/
...
if [ -L "$OLD_NAMESPACED" ]; then
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    rm -- "$OLD_NAMESPACED"
    echo "  [v1.1.3.0] Removed stale ~/.claude/skills/gstuck/output/gstack/checkpoint symlink."
    removed_any=1
  else
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    ...
    echo "  [v1.1.3.0] Removed stale ~/.claude/skills/gstuck/output/gstack/checkpoint/ (replaced by context-save + context-restore)."
    removed_any=1
  else

...
```bash
jq -r '"--- comment id \(.id) (\(.path // "top-level")) ---\n\(.body)"' /tmp/greptile_line.json | ~/.claude/skills/gstuck/output/gstack/bin/gstack-issue-guard --stdin --source greptile-line 2>/dev/null || true
jq -r '"--- comment id \(.id) (top-level) ---\n\(.body)"' /tmp/greptile_top.json | ~/.claude/skills/gstuck/output/gstack/bin/gstack-issue-guard --stdin --source greptile-top 2>/dev/null || true
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/gstuck/output/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
